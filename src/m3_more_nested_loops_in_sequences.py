...
def run_test_largest_number():
    ...
    expected = 804
    answer = largest_number(([803], [804], [2]))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    expected = -1
    answer = largest_negative_number(([-22222222], [-1], [99999]))
    print('expected and actual are:', expected, answer)
    ...
    expected = -6.7
    answer = largest_negative_number(([-6.7], [0], [-7.6]))
    print('expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    first_seq = seq_seq[0]
    for j in range(1, len(seq_seq), 1):
        seq = seq_seq[j]
        for k in range(len(first_seq)):
            for i in range(len(seq)):
                if first_seq[k] == seq[i]:
                    return True
    return False
